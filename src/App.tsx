...
function SubscriptionPlans({ open, onClose }: { open: boolean, onClose: () => void }) { const plans = [['شخصي', 'للأفراد والباحثين عن مساعدة قانونية ذكية', '١٤٩', ['تحليل المستندات', 'المساعد القانوني الذكي', 'ملخصات ومخاطر واضحة']], ['مهني', 'للمحامين والممارسين القانونيين', '٢٩٩', ['كل مزايا الشخصي', 'مراجعة العقود المتقدمة', 'سجل ومراجع لكل قرار']], ['فريق', 'للمكاتب والفرق القانونية', '٥٩٩', ['مساحات عمل متعددة', 'صلاحيات للفريق', 'دعم وأمان مؤسسي']]] as const; if (!open) return null; return <div className="subscription-modal-backdrop" role="presentation" onClick={onClose}><section className="subscription-plans subscription-modal" id="الاشتراك" role="dialog" aria-modal="true" aria-labelledby="subscription-title" onClick={event => event.stopPropagation()}><button className="subscription-modal-close" type="button" aria-label="إغلاق خطط الاشتراك" onClick={onClose}><X size={19}/></button><div className="section-intro centered"><span className="eyebrow">خطط اشتراك مرنة</span><h2 id="subscription-title">اختر مساحة العمل<br/>المناسبة لك.</h2><p>سواء كنت محاميًا أو تستخدم المنصة لإدارة مستنداتك الشخصية، كل خطة تمنحك أدوات قانونية ذكية باشتراك واضح.</p></div><div className="plan-grid">{plans.map(([name,description,price,items], index) => <article className={`plan-card ${index===1?'featured':''}`} key={name}>{index===1 && <span className="plan-badge">الأكثر اختيارًا للمحامين</span>}<span className="plan-audience">{name}</span><h3>{name === 'مهني' ? 'الخطة المهنية' : `الخطة ${name}`}</h3><p>{description}</p><div className="plan-price"><b>{price}</b><span>جنيه / شهريًا</span></div><ul>{items.map(item => <li key={item}><Check size={16}/>{item}</li>)}</ul><a className="button" href="#ابدأ">ابدأ الاشتراك <ArrowLeft size={17} aria-hidden="true"/></a></article>)}</div></section></div> }
...
export default function App() { const [plansOpen, setPlansOpen] = useState(false); useEffect(() => { const syncPlans = () => setPlansOpen(window.location.hash === '#الاشتراك'); syncPlans(); window.addEventListener('hashchange', syncPlans); return () => window.removeEventListener('hashchange', syncPlans) }, []); useEffect(() => { document.body.style.overflow = plansOpen ? 'hidden' : ''; return () => { document.body.style.overflow = '' } }, [plansOpen]); const closePlans = () => { setPlansOpen(false); if (window.location.hash === '#الاشتراك') window.history.pushState({}, '', '#الرئيسية') }; return <><a className="skip" href="#main">انتقل إلى المحتوى</a><Nav/><main id="main"><Hero/><Trust/><ReferenceScrollVisuals/><Workflow/><Problems/><Features/><ProductShowcase/><AnalysisShowcase/><AssistantShowcase/><Security/><Steps/><FAQ/><CTA/></main><SubscriptionPlans open={plansOpen} onClose={closePlans}/><Footer/></> }
